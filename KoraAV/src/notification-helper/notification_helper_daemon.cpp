...
int main() {
    // Use XDG_RUNTIME_DIR (secure, per-user location)
    // Installer adds koraav to user's group so daemon can access
    ...
    if (runtime_dir) {
        ...
        uid_t uid = getuid();
        socket_path = "/run/user/" + std::to_string(uid) + "/koraav-notifications.sock";
    }
    
    std::cout << "Starting KoraAV notification helper" << std::endl;
