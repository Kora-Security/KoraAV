...
namespace koraav {
    namespace scanner {
        ...
        bool FileScanner::Initialize(const ScanConfig& config) {
            ...
            if (config_.use_yara) {
                YaraManager& yara = YaraManager::Instance();
                if (!yara.IsReady()) {
                    yara.Initialize();
                    if (!yara.LoadRules("/opt/koraav/share/signatures/yara-rules/")) {
        ...
        FileScanResult FileScanner::ScanFile(const std::string& path) {
            ...
            try {
                ...
                if (config_.use_yara && YaraManager::Instance().IsReady()) {
        ...
        bool FileScanner::CheckYaraRules(const std::vector<char>& data, FileScanResult& result) {
            auto matches = YaraManager::Instance().ScanMemory(data.data(), data.size());
        ...
        bool FileScanner::CheckYaraRulesFile(const std::string& path, FileScanResult& result) {
            auto matches = YaraManager::Instance().ScanFile(path);
