...
{
  // ═══════════════════════════════════════════════════════════════
  // CRITICAL: Must find and connect to user's DBUS session bus
  // The daemon runs as 'koraav' user without $DISPLAY or
  // $DBUS_SESSION_BUS_ADDRESS We need to manually find the user's session bus
  // ═══════════════════════════════════════════════════════════════

  DBusError error;
  dbus_error_init(&error);

  // First, get the logged-in user and their session bus address
  std::string username = GetLoggedInUser();
  if (username.empty()) {
    std::cerr << "[DBUS DEBUG] No logged-in user found" << std::endl;
    return false;
  }

  std::cout << "[DBUS DEBUG] Found logged-in user: " << username << std::endl;

  // Get user's DBUS session address
  std::string user_dbus = GetUserDBusAddress();
  if (user_dbus.empty()) {
    std::cerr << "[DBUS DEBUG] Could not determine DBUS address for user"
              << std::endl;
    return false;
  }

  std::cout << "[DBUS DEBUG] User DBUS address: " << user_dbus << std::endl;

  // Set the environment variable so libdbus knows where to connect
  setenv("DBUS_SESSION_BUS_ADDRESS", user_dbus.c_str(), 1);

  // Now try to connect to session bus
  ...
  // Test if we can actually reach the notification service
  DBusMessage* msg =
  ...
  if (msg) {
    ...
    if (dbus_error_is_set(&error)) {
      std::cerr << "[DBUS DEBUG] Notification service not reachable: "
    ...
    if (reply) {
      ...
      std::cout << "[DBUS DEBUG] Notification service is reachable"
                << std::endl;
    }
  }
