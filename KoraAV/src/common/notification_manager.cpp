                                          ...
                                          Urgency urgency)
{
  ...
  std::string dbus_address =
    "unix:path=/run/user/" + std::to_string(pw->pw_uid) + "/bus";
  ...
  // Use runuser (systemd) or sudo -u for passwordless switching
  std::string switch_user_cmd;
  if (system("command -v runuser >/dev/null 2>&1") == 0) {
    switch_user_cmd = "runuser -u " + username + " --";
  } else {
    switch_user_cmd = "sudo -u " + username;
  }

  gdbus_cmd << switch_user_cmd << " "
  ...
  if (pipe) {
    ...
    if (result == 0 && output.find("uint32") != std::string::npos) {
      std::cout << "[NOTIFICATION DEBUG] Desktop notification sent via gdbus"
                << std::endl;
      return true;
    }
  ...
  notify_cmd << "\"" << escaped_title << "\" " << "\"" << escaped_message
             << "\" " << "2>&1";
  ...
  if (pipe) {
    ...
    if (result == 0) {
      std::cout
        << "[NOTIFICATION DEBUG] Desktop notification sent via notify-send"
        << std::endl;
      return true;
    }
